...
class IndexSelect(RelProp):
    ...
    def relprop(self, R, alpha):
        print('# In IndexSelect.relprop(R, alpha)')
        print('''Z = self.forward(self.X, self.dim, self.indices)
        S = safe_divide(R, Z)
        C = self.gradprop(Z, self.X, S)''')
...
class Clone(RelProp):
    ...
    def relprop(self, R, alpha):
        print('#- In DETR.transformer.Clone.relprop(R,alpha)')
        Z = []
        ...
        print('shape of Z', S.shape)
        print('self.gradprop(Z, self.X, S)[0]')
        C = self.gradprop(Z, self.X, S)[0]
        print('shape of C', C.shape)
        R = self.X * C
        print('#- Out DETR.transformer.Clone.relprop. output : R\nshape of R : ', R.shape)
        return R
...
class Linear(nn.Linear, RelProp):
    def relprop(self, R, alpha):
        print('# In Linear.relprop(R, alpha)')
        ...
        print('shape of self.X : ', X.weight.shape)
        pw = torch.clamp(self.weight, min=0)
        nw = torch.clamp(self.weight, max=0)
        px = torch.clamp(self.X, min=0)
        nx = torch.clamp(self.X, max=0)
        print('torch.clamp(self.weight(or self.X), min(or max))')
        print('shape of self.pw : ', pw.shape)
        print('shape of self.nw : ', nw.shape)
        print('shape of self.px : ', px.shape)
        print('shape of self.nx : ', nx.shape)
        ...
        def f(w1, w2, x1, x2):
            ...
            return C1 + C2
        print('pass function f(w1, w2, x1, x2)')
        activator_relevances = f(pw, nw, px, nx)
        inhibitor_relevances = f(nw, pw, px, nx)
        
        print('shape of activator_relevances : ', activator_relevances.shape)
        print('shape of inhibitor_relevances : ', inhibitor_relevances.shape)
        print('R_out = alpha * activator_relevances - beta * inhibitor_relevances')
        R_out = alpha * activator_relevances - beta * inhibitor_relevances
        print('shape of R_out : ', R_out.shape)
        R_out = R_out * safe_divide(R.sum(), R_out.sum())
        print('shape of R_out : ', R_out.shape)
        print('# Out Linear.relprop. output : R_out\nshape of R_out : ', R_out.shape)
        return R_out
