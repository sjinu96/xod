...
class DETR(nn.Module):
    ...
    def relprop(self, cam=None, alpha=1, **kwargs):
        print('## In DETR.relprop(cam=None, alpha=1)')
        model_predictions = self.index_select.Y
        print('shape of model_predictions', model_predictions.shape)
        print('cam = torch.zeros_like(model_prediction)')
        ...
        if kwargs["target_class"] is None:
            ...
            print('kwargs[target_class] : ', kwargs["target_class"])
        
        
                  
        
        print("kwargs[\"target_index\"] : ", kwargs["target_index"])
        print('cam[0, 0, kwargs[\"target_index\"], kwargs[\"target_class\"] = 1')
        cam[0, 0, kwargs["target_index"], kwargs["target_class"]] = 1
                  
        print('# Start DETR.index_select.reprop(cam, alpha)')
        cam = self.index_select.relprop(cam, alpha)
        print('shape of cam : ', cam.shape)
        print('# Start self.class_embed.relprop(cam, alpha)')
        cam = self.class_embed.relprop(cam, alpha)
        print('shape of cam : ', cam.shape)
        mem_zero = torch.zeros(self.memory_shape).to(cam.device)
        print('shape of mem_zero: ', mem_zero.shape)
        print('# Start DETR.transformer.relprop([cam, mem_zero], alpha)')
        cam = self.transformer.relprop([cam, mem_zero], alpha)
              
        print('shape of cam : ', cam.shape)
              
        print('## out DETR.relprop. output : cam\nshape of cam : ', cam.shape)
        return cam
